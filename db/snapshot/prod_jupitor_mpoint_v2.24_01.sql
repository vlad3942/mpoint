...
INSERT INTO client.additionalproperty_tbl (key, value, enabled, externalid, type, scope) VALUES('CLIENT_ID', '61b27ce1ca6d4a09ab9283cc052d7a36', true, (SELECT ID FROM client.merchantaccount_tbl WHERE pspid = 67 and clientid = 10077), 'merchant', 1);
INSERT INTO client.additionalproperty_tbl (key, value, enabled, externalid, type, scope) VALUES('CLIENT_SECRET', 'YbgUJ7DZf0agz8dY', true, (SELECT ID FROM client.merchantaccount_tbl WHERE pspid = 67 and clientid = 10077), 'merchant', 1);
...
VALUES('invoiceidrule', 'invoiceid ::= (psp-config/@id)=="24"OR(psp-config/@id)=="40"=(transaction.@id)', true, 10077, 'client', 0);
