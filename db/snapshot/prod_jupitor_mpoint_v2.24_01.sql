...
INSERT INTO client.cardaccess_tbl (id,clientid, cardid, enabled, pspid, countryid, stateid, "position", preferred, psp_type, installment, capture_type, walletid, capture_method, dccenabled) VALUES((SELECT max(id)+1 from client.cardaccess_tbl),10077, 93, true, 40, 640, 1, NULL, false, 3, 0, 2, NULL, 0, false);
...
INSERT INTO client.cardaccess_tbl (id,clientid, cardid, enabled, pspid, countryid, stateid, "position", preferred, psp_type, installment, capture_type, walletid, capture_method, dccenabled) VALUES((SELECT max(id)+1 from client.cardaccess_tbl),10077, 94, true, 67, 640, 1, NULL, false, 4, 0, 2, NULL, 0, false);


--Table Name : Client.MerchantAccount_Tbl

INSERT INTO Client.MerchantAccount_Tbl (id,clientid, pspid, name, username, passwd, enabled, stored_card) VALUES ((SELECT MAX(id)+1 FROM Client.MerchantAccount_Tbl) , 10077, 67, 'c636da2c-cd58-46e0-9821-8277e2b9bbde', '14db420a-dd9e-4ffb-aa8d-ed63c73dad3b', 'dOFP-WHyKlePySZ5', true, null);


--Table Name : Client.MerchantSubAccount_Tbl

INSERT INTO client.merchantsubaccount_tbl (id,accountid, pspid, "name", enabled) VALUES((SELECT max(id)+1 FROM client.merchantsubaccount_tbl),100770, 67, '-1', true);
