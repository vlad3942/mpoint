...
--Sandbox env details. Google Pay and Apple Pay MID config
-- please edit client id for onboarding.
INSERT INTO client.additionalproperty_tbl (key, value, enabled, externalid, type, scope) select 'GlobalPayment.rule', 'username ::= (property[@name=''<uname>''])
...
othercurrency ::= "GlobalPayment.Wallet.USERNAME."(transaction.@wallet-id)
password ::= (property[@name=''<passwd>''])
passwd ::= (authorized-amount/@currency)=="PHP"=<keywithcurrency>:<keywithoutcurrency>
keywithcurrency ::= "GlobalPayment.Wallet.PASSWORD."(transaction.@wallet-id)".PHP"
keywithoutcurrency ::= "GlobalPayment.Wallet.PASSWORD."(transaction.@wallet-id)
merchantaccount ::= (property[@name=''<mid>''])
