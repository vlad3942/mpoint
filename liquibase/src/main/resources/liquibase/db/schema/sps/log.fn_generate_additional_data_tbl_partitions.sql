-- select * from log.fn_generate_additional_data_tbl_partitions('log.txnpassbook_tbl',10018,'Y',20000001,30000000,1000000);
...
CREATE
OR REPLACE FUNCTION log.fn_generate_additional_data_tbl_partitions
(
...
v_p_orig_table
varchar(50):=p_ods_table;
v_context
varchar(200):=p_context;
v_period_from
varchar(50):=p_period_from;
v_period_to
varchar(50):=p_period_to;
v_error_info
text;
...
CREATE TABLE '
    ...
    ||' PARTITION OF '
    ||v_p_orig_table||' FOR VALUES IN
(
    '||v_context||'
)';
RAISE
NOTICE 'create_query : %',create_query ;
...
EXCEPTION
		   ...
		   WHEN OTHERS THEN
				...
				RAISE
WARNING '[log.fn_generate_additional_data_tbl_partitions] - UDP ERROR [OTHER] - SQLSTATE: %, SQLERRM: %', SQLSTATE, SQLERRM;
