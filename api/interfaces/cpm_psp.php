				...
				if ($code == 200)
				{
					...
					if ( (integer)$obj_Txn["id"] == $this->getTxnInfo()->getID() )
					{
						...
						else if ($iStatusCode == Constants::i3D_SECURE_ACTIVATED_STATE)
						{
							...
							$txnPassbookObj->updateInProgressOperations($iAmount, Constants::iPAYMENT_REFUNDED_STATE, Constants::sPassbookStatusInProgress);
						}
	...
	public function redeem(string $iVoucherID, float $iAmount=-1)
	{
		...
		if ($code == 200)
		{
			...
			if (isset($obj_XML->voucher->status["code"]) === true && strlen($obj_XML->voucher->status["code"]) > 0)
			{
				...
				if(empty($obj_XML->{'external-id'}) === false){
					$code = (string)$obj_XML->{'external-id'};
				}
			}
