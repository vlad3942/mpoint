<?php
/**
 * The Payment Method sub-package provides methods for retrieving payment method data
 *
...
class PaymentMethod extends EndUserAccount
{
	...
	/**
	 * Fetch payment methods for all Cards that are available to the Client.
	...
	public function getpaymentmethods($amount)
	{
		...
		$sql = "SELECT DISTINCT C.position, C.id, C.name, C.minlength, C.maxlength, C.cvclength,
					PSP.id AS pspid, MA.name AS account, PC.name AS currency,
					CA.stateid, CA.position AS client_position
				...
				INNER JOIN System.PSP_Tbl PSP ON MA.pspid = PSP.id AND CA.pspid = PSP.id AND PSP.enabled = '1'
...
//  accountid="'. $this->_obj_TxnInfo->getAccountID() .'"
		$xml = '<cards>';
...
//					echo $sql ."\n";
				...
				$xml .= '<item id="'. $RS["ID"] .'" type-id="'. $RS["ID"] .'" pspid="'. $RS["PSPID"] .'" min-length="'. $RS["MINLENGTH"] .'" max-length="'. $RS["MAXLENGTH"] .'" cvc-length="'. $RS["CVCLENGTH"] .'" state-id="'. $RS["STATEID"] .'">';
