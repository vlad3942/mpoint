...
class ClientConfig extends BasicConfig
{
	...
	public static function produceConfig(RDB &$oDB, $id, $acc, $kw=-1)
	{
		...
		$sql = "SELECT Cl.id AS clientid, Cl.name AS client, Cl.flowid, Cl.username, Cl.passwd,
					...
					C.id AS countryid, C.name AS country, C.currency, C.maxbalance, C.minmob, C.maxmob, C.channel,
		...
		$sql .= " {ACCOUNT CLAUSE}
				GROUP BY Cl.id, Cl.name, Cl.flowid, Cl.username, Cl.passwd,
					...
					Cl.maxamount, Cl.lang, Cl.terms,
					Cl.mode, Cl.auto_capture, Cl.send_pspid, Cl.store_card,
					C.id, C.name, C.currency, C.maxbalance, C.minmob, C.maxmob, C.channel,
					C.priceformat, C.decimals, C.als, C.doi,
...
//		echo $sql ."\n";
		...
		$obj_CountryConfig = new CountryConfig($RS["COUNTRYID"], $RS["COUNTRY"], $RS["CURRENCY"], $RS["MAXBALANCE"], $RS["MINMOB"], $RS["MAXMOB"], $RS["CHANNEL"], $RS["PRICEFORMAT"], $RS["DECIMALS"], $RS["ALS"], $RS["DOI"]);
