...
class TxnInfo {
	 ...
	 *        	Total amount the customer has been refunded for the Transaction
	 * @param string $orid
	 *        	Clients Order ID of the Transaction
	 * @param string $extid
	 *        	External ID of the Transaction (usually the txn ref of the PSP)
	 * @param string $addr
	 *        	Customer's Mobile Number (MSISDN)
	 * @param integer $oid
	 *        	GoMobile's ID for the Customer's Mobile Network Operator
	 ...
	 *        	Absolute URL to the Client's My Account Icon
	 * @param string $aurl
	 *        	Absolute URL to the external system where a customer may be authenticated. This is generally an existing e-Commerce site or a CRM system
	 * @param string $l
	 *        	The language that all payment pages should be rendered in by default for the Client
	 * @param integer $m
	 *        	The Client Mode in which the Transaction should be Processed
	 * @param boolean $ac
	 *        	Boolean Flag indicating whether Auto Capture should be used for the transaction
	 * @param integer $accid
	 *        	Unique ID for the End-User's prepaid account that the transaction should be associated with
	 * @param string $cr
	 *        	The Client's Reference for the Customer
	 * @param integer $gmid
	 *        	GoMobile's Unique ID for the MO-SMS that was used to start the payment transaction. Defaults to -1.
	 * @param boolean $asc
	 *        	Boolean Flag indicating whether the "Save Card Info" box should automatically be checked on the payment page
	 * @param string $mrk
	 *        	String indicating the markup language used to render the payment pages
	 * @param string $desc
	 *        	String that holds the description of an order
	 * @param string $ip
	 *        	String that holds the customers IP address
	 * @param integer $pspid
	 *        	Unique ID for the The PSP used for the transaction Defaults to -1.
	 * @param integer $fee
	 *        	The amount the customer will pay in fee´s for the Transaction.
	 * @param long $cptamt
	 *        	The Full amount that has been captured for the Transaction
	...
	public function getAuthenticationURL() {
		return $this->_sAuthenticationURL;
	}
	/**
	 ...
	 * @return boolean
	 */
	public function useAutoCapture() {
		return $this->_bAutoCapture;
	}
	/**
	 ...
	 * @return boolean
	 */
	public function autoStoreCard() {
		return $this->_bAutoStoreCard;
	}
	/**
	...
	public function getHMAC() {
		return sha1 ( $this->_obj_ClientConfig->getID () . $this->_obj_ClientConfig->getAccountConfig ()->getID () . $this->_iID . $this->_sOrderID . $this->_obj_CountryConfig->getID () . $this->_lAmount . $this->_sCustomerRef . $this->_sEMail . $this->_sMobile . $this->_obj_ClientConfig->getPassword () );
	}
	/**
	 ...
	 *
	 * @param integer $id
	 *        	Unique ID for the End-User's prepaid account
	 */
	public function setAccountID($id) {
		$this->_iAccountID = ( integer ) $id;
	}
	/**
	 ...
	 *
	 * @param UAProfile $oUA
	 *        	Reference to the User Agent Profile for the Customer's Mobile Device (optional)
	 * @return string
	 */
	public function toXML(UAProfile &$oUA = null) {
		if (is_null ( $oUA ) === false && strlen ( $this->_sLogoURL ) > 0) {
			$obj_Image = new Image ( $this->_sLogoURL );
			if ($oUA->getHeight () * iCLIENT_LOGO_SCALE / 100 < $obj_Image->getSrcHeight ()) {
				$iHeight = $oUA->getHeight () * iCLIENT_LOGO_SCALE / 100;
			} else {
				$iHeight = $obj_Image->getSrcHeight ();
			}
	...
	public static function produceInfoFromOrderNoAndMerchant(RDB $obj, $orderNo, $merchant = '', array $data = array()) {
		...
		if (($obj_TxnInfo instanceof TxnInfo) === false) {
			throw new TxnInfoException ( "Transaction with orderno: " . $orderNo . " not found", 1001 );
		}
	 ...
	 *
	 * @param integer $id
	 *        	Unique ID for the Transaction that should be instantiated
	 * @param [TxnInfo|ClientConfig|RDB] $obj
	 *        	Reference to one of the following objects:
	 ...
	 * @param array $misc
	 *        	Reference to array of miscelaneous data that is used for instantiating the data object with the Transaction Information
	 * @return TxnInfo
	...
	public static function produceInfo($id, &$obj, array &$misc = null) {
		...
		switch (true) {
			case ($obj instanceof TxnInfo) : // Instantiate from array of new Transaction Information
				...
				if (array_key_exists ( "typeid", $misc ) === false) {
					$misc ["typeid"] = $obj->getTypeID ();
				}
				if (array_key_exists ( "client-config", $misc ) === false) {
					$misc ["client-config"] = $obj->getClientConfig ();
				}
				if (array_key_exists ( "country-config", $misc ) === false) {
					$misc ["country-config"] = $obj->getCountryConfig ();
				}
				if (array_key_exists ( "card-id", $misc ) === false) {
					$misc ["card-id"] = $obj->getCardID ();
				}
				if (array_key_exists ( "amount", $misc ) === false) {
					$misc ["amount"] = $obj->getAmount ();
				}
				if (array_key_exists ( "points", $misc ) === false) {
					$misc ["points"] = $obj->getPoints ();
				}
				if (array_key_exists ( "reward", $misc ) === false) {
					$misc ["reward"] = $obj->getReward ();
				}
				if (array_key_exists ( "orderid", $misc ) === false) {
					$misc ["orderid"] = $obj->getOrderID ();
				}
				if (array_key_exists ( "extid", $misc ) === false) {
					$misc ["extid"] = $obj->getExternalID ();
				}
				if (array_key_exists ( "mobile", $misc ) === false) {
					$misc ["mobile"] = $obj->getMobile ();
				}
				if (array_key_exists ( "operator", $misc ) === false) {
					$misc ["operator"] = $obj->getOperator ();
				}
				if (array_key_exists ( "email", $misc ) === false) {
					$misc ["email"] = $obj->getEMail ();
				}
				if (array_key_exists ( "logo-url", $misc ) === false) {
					$misc ["logo-url"] = $obj->getLogoURL ();
				}
				if (array_key_exists ( "css-url", $misc ) === false) {
					$misc ["css-url"] = $obj->getCSSURL ();
				}
				if (array_key_exists ( "accept-url", $misc ) === false) {
					$misc ["accept-url"] = $obj->getAcceptURL ();
				}
				if (array_key_exists ( "cancel-url", $misc ) === false) {
					$misc ["cancel-url"] = $obj->getCancelURL ();
				}
				if (array_key_exists ( "callback-url", $misc ) === false) {
					$misc ["callback-url"] = $obj->getCallbackURL ();
				}
				if (array_key_exists ( "icon-url", $misc ) === false) {
					$misc ["icon-url"] = $obj->getIconURL ();
				}
				if (array_key_exists ( "language", $misc ) === false) {
					$misc ["language"] = $obj->getLanguage ();
				}
				if (array_key_exists ( "mode", $misc ) === false) {
					$misc ["mode"] = $obj->getMode ();
				}
				if (array_key_exists ( "auto-capture", $misc ) === false) {
					$misc ["auto-capture"] = $obj->useAutoCapture ();
				}
				if (array_key_exists ( "gomobileid", $misc ) === false) {
					$misc ["gomobileid"] = $obj->getGoMobileID ();
				}
				if (array_key_exists ( "accountid", $misc ) === false) {
					$misc ["accountid"] = $obj->getAccountID ();
				}
				if (array_key_exists ( "customer-ref", $misc ) === false && strlen ( $obj->getCustomerRef () ) > 0) {
					$misc ["customer-ref"] = $obj->getCustomerRef ();
				}
				if (array_key_exists ( "markup", $misc ) === false) {
					$misc ["markup"] = $obj->getMarkupLanguage ();
				}
				if (array_key_exists ( "auto-store-card", $misc ) === false) {
					$misc ["auto-store-card"] = false;
				}
				if (array_key_exists ( "refund", $misc ) === false) {
					$misc ["refund"] = 0;
				}
				if (array_key_exists ( "auth-url", $misc ) === false) {
					$misc ["auth-url"] = $obj->getAuthenticationURL ();
				}
				if (array_key_exists ( "description", $misc ) === false) {
					$misc ["description"] = $obj->getDescription ();
				}
				if (array_key_exists ( "ip", $misc ) === false) {
					$misc ["ip"] = $obj->getIP ();
				}
				if (array_key_exists ( "psp-id", $misc ) === false) {
					$misc ["psp-id"] = $obj->getPSPID ();
				}
				if (array_key_exists ( "fee", $misc ) === false) {
					$misc ["fee"] = $obj->getFee ();
				}
				if (array_key_exists ( "captured-amount", $misc ) === false) {
					$misc ["captured-amount"] = $obj->getCapturedAmount ();
				}
			...
			case ($obj instanceof RDB) : // Instantiate from Transaction Log
				...
				// Transaction found
				if (($obj_TxnInfo instanceof TxnInfo) === false) {
					throw new TxnInfoException ( "Transaction with ID: " . $id . " not found" . $sDebug, 1001 );
				}
				break;
	...
	public function hasEitherState(RDB $obj_DB, $aStateID) {
		...
		if ($res === false) {
			trigger_error ( "Failed to determine whether transaction #" . $this->getID () . " has states: " . $sStates, E_USER_WARNING );
		}
		...
		if (is_array ( $aOrderData ) === true) {
			foreach ( $aOrderData as $aOrderDataObj ) {
				...
				if (is_array ( $RS ) === false) {
					throw new mPointException ( "Unable to generate new Order ID", 1001 );
				}
				...
				if (is_resource ( $obj_DB->query ( $sql ) ) === false) {
					if (is_array ( $RS ) === false) {
						throw new mPointException ( "Unable to insert new record for Order: " . $RS ["ID"], 1002 );
					}
				} else {
	 ...
	 *        	Data object with the Shipping Address Data details
	 *        	
	 */
	public function setShippingDetails(RDB $obj_DB, $aShippingData) {
		if (is_array ( $aShippingData ) === true) {
			foreach ( $aShippingData as $aShippingObj ) {
				...
				// Error: Unable to generate a new Order ID
				if (is_array ( $RS ) === false) {
					throw new mPointException ( "Unable to generate new address ID", 1001 );
				}
				...
				if (is_resource ( $obj_DB->query ( $sql ) ) === false) {
					if (is_array ( $RS ) === false) {
						throw new mPointException ( "Unable to insert new record for Address: " . $RS ["ID"], 1002 );
					}
				} else {
	 ...
	 *        	Data object with the Additional Data details
	 *        	
	 */
	public function setAdditionalDetails(RDB $obj_DB, $aAdditionalData, $ExternalID) {
		...
		if (is_array ( $aAdditionalData ) === true) {
			foreach ( $aAdditionalData as $aAdditionalDataObj ) {
				...
				if (is_array ( $RS ) === false) {
					throw new mPointException ( "Unable to generate new Additional Data ID", 1001 );
				}
	 ...
	 *        	Data object with the flight details
	 * @param Array $aAdditionalDatas
	 *        	Data object with the Additional data details
	 *        	
	 */
	public function setFlightDetails(RDB $obj_DB, $aFlightData, $aAdditionalDatas) {
		...
		if (is_array ( $aFlightData ) === true) {
			...
			if (is_array ( $RS ) === false) {
				throw new mPointException ( "Unable to generate new Flight ID", 1001 );
			}
			...
			if (is_resource ( $obj_DB->query ( $sql ) ) === false) {
				if (is_array ( $RS ) === false) {
					throw new mPointException ( "Unable to insert new record for Flight: " . $RS ["ID"], 1002 );
				}
			} else {
				$aReturnValue = $RS ["ID"];
			}
	 ...
	 *        	Data object with the passenger details
	 * @param Array $aAdditionalDatas
	 *        	Data object with the Additional data details
	 *        	
	 */
	public function setPassengerDetails(RDB $obj_DB, $aPassengerData, $aAdditionalDatas) {
		...
		if (is_array ( $aPassengerData ) === true) {
			...
			if (is_array ( $RS ) === false) {
				throw new mPointException ( "Unable to generate new Passenger ID", 1001 );
			}
			...
			if (is_resource ( $obj_DB->query ( $sql ) ) === false) {
				if (is_array ( $RS ) === false) {
					throw new mPointException ( "Unable to insert new record for Passenger: " . $RS ["ID"], 1002 );
				}
			} else {
				$aReturnValue = $RS ["ID"];
			}
