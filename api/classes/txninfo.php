...
//		echo $sql;
	...
	public function getPaymentStatus($_OBJ_DB,int $txnId,$linkedTxnId): string
	{
		...
		if($auto_capture == AutoCaptureType::eRunTimeAutoCapt){
			...
			$checkTxnStatus = $this->checkTxnStatus($_OBJ_DB,Constants::iPAYMENT_ACCEPTED_STATE,$txnId);
		}else{
            //if not manual capture then check for 2001 is logged and fraud states are not logged, if so payment is complete
			$checkTxnStatus = $this->checkTxnStatus($_OBJ_DB,Constants::iPAYMENT_CAPTURED_STATE,$txnId);
		}
		if($checkTxnStatus > 0){
			$paymentStatus = 'Complete';
		}
		// check if 2010 is logged and fraud state is logged
        $checkFailedTxn = $this->checkTxnStatus($_OBJ_DB,Constants::iPAYMENT_REJECTED_STATE,$txnId,true);
		if($checkFailedTxn > 0){
            $paymentStatus = 'Failed';
        }
        // check both the transaction status to get payment status for combined txns
		if($linkedTxnId > 0){
			$checkLinkedTxnStatus = $this->getPaymentStatus($_OBJ_DB,$linkedTxnId,0);
	...
	public function checkTxnStatus($_OBJ_DB,int $stateId,int $txnId,$is_failed=false): int
	{
	    $sql = "SELECT COUNT(id) AS C
			FROM Log".sSCHEMA_POSTFIX.".Message_Tbl
	...
	public function getLinkedTransactions($_OBJ_DB,int $linkedTxnId) : string
	{
		$linkedTxnData     = [$this->getID(),$linkedTxnId];
		...
		foreach($linkedTxnData as $linkedTxn){
			...
			$linkedTxnXml  .= "<status>".$this->getPaymentStatus($_OBJ_DB,$linkedTxn,0)."</status>";
			$linkedTxnXml  .= "</transaction_details>";
		}
		$linkedTxnXml .= "</linked_transactions>";
		return $linkedTxnXml;
