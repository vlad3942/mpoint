		...
		else
		{
			$sql = "SELECT Txn.enabled, Msg.stateid
					...
					WHERE Txn.id = ". intval($mpointid) ." AND Txn.clientid = ". intval($clientid) ."
						AND (Msg.stateid >= ". Constants::iPAYMENT_ACCEPTED_STATE ." AND  Msg.stateid != ". Constants::iPAYMENT_WITH_ACCOUNT_STATE ." OR  Msg.stateid = ". Constants::iPAYMENT_ACCOUNT_VALIDATED .")
