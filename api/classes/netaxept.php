...
class NetAxept extends Callback {
	 ...
	 *
	 * @param HTTPConnInfo $oCI
	 *        	on how to connect to NetAxept
	 * @param integer $merchant
	 *        	merchant ID to identify us to NetAxept
	 * @param integer $account        	
	 * @param String $currency
	 *        	currency to use in ISO 4217 format
	 * @param inteter $cardid
	 *        	card ID
	 * @return String information of how the client should submit card details to NetAxepts server
	 * @throws E_USER_WARNING
	 */
	public function initialize(HTTPConnInfo &$oCI, $merchant, $account, $currency, $cardid, $storecard) {
		$obj_SOAP = new SOAPClient ( "https://" . $oCI->getHost () . $oCI->getPath (), array (
				"trace" => true,
				"exceptions" => true 
		) );
		$sOrderNo = $this->getTxnInfo ()->getOrderID ();
		if (empty ( $sOrderNo ) === true) {
			$sOrderNo = $this->getTxnInfo ()->getID ();
		}
		
		$request = array (
				"Description" => "mPoint Transaction: " . $this->getTxnInfo ()->getID () . " for Order: " . $this->getTxnInfo ()->getOrderID (),
				"Environment" => array (
		...
		try {
			...
			if (intval ( $obj_Std->RegisterResult->TransactionId ) == $this->getTxnInfo ()->getID ()) {
				...
				$data = array (
						"psp-id" => Constants::iNETAXEPT_PSP,
	 ...
	 *        	ID previously returned by NetAxept during authorisation
	 * @return String
	 * @throws E_USER_WARNING
	 */
		...
		try {
			...
			if ($obj_Std->ProcessResult->ResponseCode == 'OK') {
				...
				if ($queryResponse->Summary->Authorized == "true") {
					$iStateID = $this->completeTransaction ( Constants::iNETAXEPT_PSP, $transactionID, $this->getCardID ( $queryResponse->CardInformation->Issuer ), Constants::iPAYMENT_ACCEPTED_STATE, array (
							'0' => var_export ( $obj_Std->ProcessResult, true ) 
					) );
				} else {
					$iStateID = $this->completeTransaction ( Constants::iNETAXEPT_PSP, $transactionID, $this->getCardID ( $queryResponse->CardInformation->Issuer ), Constants::iPAYMENT_REJECTED_STATE, array (
							'0' => var_export ( $obj_Std->ProcessResult, true ) 
					) );
				}
			}
	 ...
	 *        	ID previously returned by NetAxept during authorisation
	 * @param integer $txn
	 *        	ID previously returned by WannaFind during authorisation
	 * @return String
	 * @throws E_USER_WARNING
	 */
	 ...
	 *
	 * @param integer $sid
	 *        	Unique ID of the State that the Transaction terminated in
	 * @param array $_post
	 *        	Array of data received from WannaFind via HTTP POST
	 */
	 ...
	 *        	ticket which references a previously stored card
	 * @return integer
	 * @throws E_USER_WARNING
	 */
