...
class SurePay extends General
{
	...
	public function notifyClient()
	{
		...
		else { $this->newMessage($this->_obj_TxnInfo->getID(), Constants::iEMAIL_REJECTED_START, $sSubject); }
	}
	...
	public static function produceSurePays(RDB &$oDB, $type)
	{
		...
		$sql = "SELECT DISTINCT Txn.id, Msg.data AS url, SP.email, Extract(epoch from Txn.created) AS created
				...
				WHERE Msg.stateid = ". Constants::iCONST_LINK_STATE ."
					AND NOT EXISTS (SELECT id
									...
									WHERE M.txnid = Txn.id AND M.stateid = ". Constants::iACTIVATE_LINK_STATE .")";
