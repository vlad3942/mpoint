	...
	public function searchTransactionLogs($sTimeZoneOffset , array $aClientIDs, array $aAccountIDs, array $aPspIDs, array $aCardIDs, array $aStateIDs, $id=-1, $ono="", CustomerInfo $oCI=null, $start="", $end="", $verbose=false, $limit=100, $offset=0)
	{
		...
		if (count($aStateIDs) == 0)
		{
			$aStateIDs = array(Constants::iINPUT_VALID_STATE,
                    ...
                    Constants::iPAYMENT_CANCEL_FAILED_STATE,
                    Constants::iPAYMENT_REFUND_FAILED_STATE,
                    Constants::iPAYMENT_REQUEST_CANCELLED_STATE,
			...
			if(in_array( $RS["ASSTATEID"], $aStateIDs ) == true)
			{
				$aObj_TransactionLogs[] = new TransactionLogInfo($RS["ID"],
						...
						CustomerInfoFactory::getInstance($this->getDBConn(), $this->getText(), new ClientURLConfig($RS["CLIENTID"], ClientConfig::iAUTHENTICATION_URL, $RS['AUTHURL']),
							$RS["CUSTOMERID"], (int)$RS["OPERATORID"]/100, $RS["MOBILE"], $RS["EMAIL"], $RS["CUSTOMER_REF"],
