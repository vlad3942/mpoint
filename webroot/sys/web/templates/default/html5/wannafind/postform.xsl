			...
			<div id="outer-border">
				<div>	
					<form id="pay-card" action="{wannafind/url}" method="post">
						...
						<div id="card-info">
							...
							<tr class="first-row">
								...
								<td class="right-column stretch" colspan="2">
									<input name="cardnum" onblur="javascript:parent.postMessage('reposition', '*');" pattern="[0-9]*" value="{$card-number}" maxlength="19" class="text" style="-wap-input-format:'*N';" type="number" />
								</td>
							</tr>
							<tr class="row">
								...
								<td class="right-column stretch" colspan="2">
									<input type="number" id="expiry-month" name="emonth" maxlength="2" value="{$expiry-month}" size="3" style="-wap-input-format:'*N';" pattern="[0-9]*" onblur="javascript:parent.postMessage('reposition', '*');" />
									<xsl:value-of select="concat(' ', '/', ' ')" />
									<input type="number" id="expiry-year" name="eyear" maxlength="2" value="{$expiry-year}" size="3" style="-wap-input-format:'*N';" pattern="[0-9]*" onblur="javascript:parent.postMessage('reposition', '*');" />
								</td>
							</tr>
							<tr class="row combined-row">
								...
								<td class="right-column combined-row stretch" colspan="2">
									...
									<xsl:when test="card/@id = 1">
										<input type="number" name="cvc" maxlength="4" value="{$cvc}" size="5" style="-wap-input-format:'*N';" pattern="[0-9]*" onblur="javascript:parent.postMessage('reposition', '*');" />
									</xsl:when>
									<xsl:otherwise>
										<input type="number" name="cvc" maxlength="3" value="{$cvc}" size="4" style="-wap-input-format:'*N';" pattern="[0-9]*" onblur="javascript:parent.postMessage('reposition', '*');" />
									</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
							...
							<xsl:if test="client-config/store-card &gt; 0">
								<tr>
									...
									<tr>
										...
										<td class="stretch"><xsl:value-of select="labels/save-card" /></td>
									</tr>
									</table>
									</td>
								</tr>
						...
						<!-- Complete Payment -->
						<div id="submit">
							<a class="submit-button" id="pay" onclick="javascript:document.getElementById('loader').style.visibility='visible'; this.className='submit-button-clicked'; this.disabled=true; document.getElementById('callbackurl').value+='&amp;authtype='+ document.getElementById('authtype').value; document.getElementById('pay-card').submit();">
