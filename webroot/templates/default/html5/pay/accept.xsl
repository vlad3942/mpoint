...
<xsl:template match="/root">
	...
	<div id="info">
		...
		<xsl:if test="client-config/email-receipt = 'true'">
			<div>
				<a onclick="document.location.href='{func:constLink('email.php') }';"><xsl:value-of select="labels/email-receipt" /></a>
			</div>
		</xsl:if>
	...
	<script type="text/javascript">
		parent.postMessage('completed,<xsl:value-of select="transaction/@id" />', '*');
	</script>
</xsl:template>
