...
<xsl:if test="count(/root/stored-cards/card) > 0">
		...
		<div class="panel panel-default add-card">
                        ...
                        <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
                          ...
                          <div class="row">
                           ...
                           <xsl:for-each select="/root/stored-cards/card">
                            ...
                            <div class="col-md-12">   
                                 <p class="remove-card">
                                    <a class="red remove-alert{@id}"><small>Remove </small> <span class="glyphicon glyphicon-trash" title="{/root/labels/delete-card}"></span>
                                    </a>
                                  </p>
                            </div>
                            </div>
          ...
          <div id="remove-card-alert{@id}" class="alert fade in alert-danger" role="alert" style="display:none">
          <div class="col-md-12">
          <div class="col-md-4">Are you sure you want to remove this card?</div>
            
         	<div class="col-md-6">
         	<form id="delete-card" class="classy-form"  action="{func:constLink('/cpm/sys/del_card.php') }" method="post">
				...
				<input type="password" class="form-control" style="display: inline;width:100px;padding: none;font-size: inherit;" name="pwd" value="" required="required" placeholder="{/root/labels/password}" />
				<button type="submit" class="btn pull-right">Yes, remove it.</button>
			</form></div>
			<div class="col-md-2"><button type="" class="btn btn-sm cancel">Cancel</button></div>
            </div>
          </div>
          <!-- End: Saved card remove alert -->
                             <!-- Start: Enter password modal -->
                  <div id="modal{@id}" class="modal fade cvv-password" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel{@id}">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content text-center">
                        <div class="modal-header">
                          <button type="button" class="close" data-dismiss="modal{@id}" aria-label="Close"><span aria-hidden="true"></span></button>
                          <h4 class="modal-title">Enter details to pay with this card</h4>
                        </div>
                        <form class="form-inline classy-form" id="pay-account" action="{func:constLink('/pay/sys/authorize.php') }" method="post">
                        	<input type="hidden" name="euaid" value="{/root/cards/@accountid}" />
                          ...
                          <label class="sr-only" for="cvv"><xsl:value-of select="/root/labels/cvv" /></label>
                            <div class="input-group">
                              <div class="input-group-addon">CVV</div>
                              <input name="cvv" type="password" class="form-control" id="exampleInputAmount"/>
                            </div>
                          </div>
                          <div class="form-group">
                          <label class="sr-only" for="password"><xsl:value-of select="/root/labels/password" /></label>
                            <div class="input-group">
                              <div class="input-group-addon">Password</div>
                              <input type="password" name="pwd" class="form-control" id="exampleInputAmount"/>
                            </div>
                          </div>
                              
                          <button type="submit" class="btn"><xsl:value-of select="/root/labels/submit" /></button>
                        </form>
                      </div>
                    </div>
                  </div>
...
<xsl:template match="cards" mode="cpm">
                     ...
                     <div class="panel panel-default add-card">
                      ...
                      <div id="collapseThree" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingThree">
                        <div class="panel-body">
                          <form class="classy-form" action="{func:constLink('/pay/sys/authorize.php') }" method="post" autocomplete="on">
                            <div class="row">
			                  ...
			                  <input type="hidden" name="euaid" value="{/root/cards/@accountid}" />
			 				  <div class="form-group col-md-12">
                                ...
                                <input type="text" name="cardholdername" class="form-control" id="card-name" placeholder="John Doe"/>
                              </div>
                               <div class="form-group col-md-12">
                                <label for="cardnumber"><xsl:value-of select="/root/labels/cardnumber" /></label>
                                <input type="tel" name="cardnumber" class="form-control cc-number" autocomplete="cc-number" maxlength="23" required="required" placeholder="1111 2222 3333 4444" />
                               </div>
                               <div class="form-group col-md-12">
							   	 <div class="col-md-3">
						         <label for="expiry-month"><xsl:value-of select="/root/labels/expiry" /></label>
						         <input type="number" name="expiry-month" class="form-control cc-month" maxlength="2" required="required" placeholder="MM" />
						         </div>
						         <div class="col-md-3">
						         <label for="expiry-year">Enter Year</label>
						         <input type="number" name="expiry-year" class="form-control cc-year" autocomplete="cc-year" maxlength="2" required="required" placeholder="YY" />
						         </div>
     						  	 <div class="form-group col-md-6">
                                <label for="cvv"><xsl:value-of select="/root/labels/cvv" /></label>
                                <input type="password" name="cvv" class="form-control cc-cvv" autocomplete="off" maxlength="4" required="required" placeholder="CVV" />
                                </div>
                               </div>
                              <div class="checkbox col-md-12">
                                <label>
                                  <input id="save-this-card"  name="store-card" type="checkbox" onchange="valueChanged()"/> Save this card details for future use.
                                </label>
                              </div>
								<xsl:choose>
								<xsl:when test="/root/cards/@accountid > 0">
								 <div id="saveNewCardName" class="form-group col-md-12" style="display: none">
								    <label for="cardname">Name this card</label>
								    <input type="text" name="cardname" class="form-control" id="exampleInputPassword1" placeholder="Name your card"/>
								 </div>
								</xsl:when>
								<xsl:otherwise>
								<div id="saveNewCardName" class="form-group col-md-12" style="display: none">
								...
								<input type="password" class="form-control password" name="new-password" maxlength="20" placeholder="{/root/labels/new-password}" title="new-password" />
								<input type="password" class="form-control password" name="repeat-password" maxlength="20" placeholder="{/root/labels/repeat-password}" title="repeat-password" />
								</div>
								</xsl:otherwise>
								</xsl:choose>
...
<a role="button" data-toggle="collapse" data-parent="#accordion1" href="#collapseTwo" aria-expanded="true" aria-controls="collapseThree" >
<h4 class="panel-title red">
<i class="more-less glyphicon glyphicon-plus"></i>
Wallets
</h4>
				...
				<div class="wallet-type" id="walletvisa_@id">
					...
					<div class="row" data-toggle="modal" data-target=".login-wallet">
						...
						<div class="col-xs-2 col-sm-2 col-md-2 text-right">
							<span class="glyphicon glyphicon-chevron-right right-icon" aria-hidden="true"></span>
						</div>
					</div>
				</div>
			</div>
