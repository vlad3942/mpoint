		...
		for ($i=0; $i<count($obj_DOM->login); $i++)
		{
			...
			if ($code == 100)
			{
				...
				if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW']) )
				{
					...
					if (count($aMsgCds) == 0)
					{
						$iAccountID = EndUserAccount::getAccountID_Static($_OBJ_DB, $obj_ClientConfig, $obj_CountryConfig, $obj_DOM->login[$i]->{'client-info'}->{'customer-ref'}, $obj_DOM->login[$i]->{'client-info'}->mobile, $obj_DOM->login[$i]->{'client-info'}->email,$obj_DOM->login[$i]->{'client-info'}["profileid"]);
						...
						if ($iAccountID > -1)
						{
                            if (count($obj_DOM->login[$i]->{'auth-token'}) == 1
                                ...
                                if ($obj_Validator->valURL($url, $obj_ClientConfig->getAuthenticationURL()) == 10) {
                                    ...
                                    $code = $obj_mPoint->auth($obj_ClientConfig, $obj_CustomerInfo, trim($obj_DOM->login[$i]->{'auth-token'}), intval($obj_DOM->login[$i]["client-id"]));
                                } else {
                                    $code = 8;
                                }
                            } else {
                            ...
                            elseif ($code == 11) {
                                ...
                                if ($code == 3) {
                                    ...
                                    $_OBJ_TXT = new api\classes\core\TranslateText(array(sLANGUAGE_PATH . $obj_DOM->login[$i]->{'client-info'}["language"] . "/global.txt", sLANGUAGE_PATH . $obj_DOM->login[$i]->{'client-info'}["language"] . "/custom.txt"), sSYSTEM_PATH, 0, "UTF-8");
