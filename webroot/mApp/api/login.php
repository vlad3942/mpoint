						...
						if ($iAccountID > -1)
						{
                            if (count($obj_DOM->login[$i]->{'auth-token'}) == 1
                                ...
                                if ($obj_Validator->valURL($url, $obj_ClientConfig->getAuthenticationURL()) == 10) {
                                    ...
                                    $code = $obj_mPoint->auth(HTTPConnInfo::produceConnInfo($url), $obj_CustomerInfo, trim($obj_DOM->login[$i]->{'auth-token'}), intval($obj_DOM->login[$i]["client-id"]));
                                } else {
                                    $code = 8;
                                }
                            } else {
                            ...
                            if ($code == 10 || ($code == 11 && $obj_ClientConfig->smsReceiptEnabled() === false)) {
                                ...
                                if (is_array($aObj_XML) === true && count($aObj_XML) > 0) {
                                    ...
                                    for ($j = 0; $j < count($aObj_XML); $j++) {
                                        $xml .= '<card id="' . $aObj_XML[$j]["id"] . '" type-id="' . $aObj_XML[$j]->type["id"] . '" psp-id="' . $aObj_XML[$j]["pspid"] . '" preferred="' . $aObj_XML[$j]["preferred"] . '" charge-type-id="' . $aObj_XML[$j]["charge-type-id"] .'" expired="' . $aObj_XML[$j]["expired"] . '">';
