	...
	if ( ($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate(sPROTOCOL_XSD_PATH ."get_payment_methods.xsd") === true && count($obj_DOM->{'get-payment-methods'}) > 0)
	{
		...
		for ($i=0; $i<count($obj_DOM->{'get-payment-methods'}); $i++)
		{
			...
			if ($code == 100)
			{
                ...
                if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW']) && $obj_ClientConfig->hasAccess($_SERVER['HTTP_X_ORIGINAL_FORWARDED_FOR']) === true)
				{
