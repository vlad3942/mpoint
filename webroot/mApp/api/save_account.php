		...
		for ($i=0; $i<count($obj_DOM->{'save-account'}); $i++)
		{
			...
			if ($code == 100)
			{
				...
				if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW']) )
				{
					...
					if (count($obj_DOM->{'save-account'}[$i]->card) == 1 && $obj_Validator->valName( (string) $obj_DOM->{'save-account'}[$i]->card) != 10) { $aMsgCds[] = $obj_Validator->valName( (string) $obj_DOM->{'save-account'}[$i]->{'card'}->name) + 50; }
