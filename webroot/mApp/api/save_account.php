...
if (array_key_exists("PHP_AUTH_USER", $_SERVER) === true && array_key_exists("PHP_AUTH_PW", $_SERVER) === true) {
    if (($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate(sPROTOCOL_XSD_PATH . "mpoint.xsd") === true && count($obj_DOM->{'save-account'}) > 0) {
        ...
        for ($i = 0; $i < count($obj_DOM->{'save-account'}); $i++) {
            // Set Global Defaults
            if (empty($obj_DOM->{'save-account'}[$i]["account"]) === true || intval($obj_DOM->{'save-account'}[$i]["account"]) < 1) {
                $obj_DOM->{'save-account'}[$i]["account"] = -1;
            }
            ...
            if ($code == 100) {
                ...
                if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW'])) {
                    ...
                    $_OBJ_TXT = new api\classes\core\TranslateText(array(sLANGUAGE_PATH . $obj_DOM->{'save-account'}[$i]->{'client-info'}["language"] . "/global.txt", sLANGUAGE_PATH . $obj_DOM->{'save-account'}[$i]->{'client-info'}["language"] . "/custom.txt"), sSYSTEM_PATH, 0, "UTF-8");
                    ...
                    if (count($obj_DOM->{'save-account'}[$i]->{'full-name'}) == 1) {
                        ...
                        if ($pos === false) {
                            $pos = strlen($obj_DOM->{'save-account'}[$i]->{'full-name'});
                        } else {
                            $obj_DOM->{'save-account'}[$i]->{'last-name'} = substr($obj_DOM->{'save-account'}[$i]->{'full-name'}, $pos + 1);
                        }
                    ...
                    if (count($aMsgCds) == 0) {
                        ...
                        if ($code == 10 || $code == 11) {
                            ...
                            if ($obj_ClientConfig->getAdditionalProperties(Constants::iInternalProperty, "ENABLE_PROFILE_ANONYMIZATION") == "true") {
                                ...
                                if (empty($obj_DOM->{'save-account'}[$i]->{'client-info'}["profileid"]) === true) {
                                    ...
                                    if ($iProfileID < 0) {
                                        ...
                                        $iProfileID = $obj_mProfile->saveProfile($obj_ClientConfig, $obj_CountryConfig->getID(), $obj_DOM->{'save-account'}[$i]->{'client-info'}->mobile, $obj_DOM->{'save-account'}[$i]->{'client-info'}->email, $obj_DOM->{'save-account'}[$i]->{'client-info'}->{'customer-ref'}, "false");
                                        if ($iProfileID < 0) {
                                    ...
                                    $iProfileID = (integer)$obj_DOM->{'save-account'}[$i]->{'client-info'}["profileid"];
                                }
                            }
