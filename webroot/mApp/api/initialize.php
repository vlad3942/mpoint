...
if (array_key_exists ( "PHP_AUTH_USER", $_SERVER ) === true && array_key_exists ( "PHP_AUTH_PW", $_SERVER ) === true) {
	if (($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate ( sPROTOCOL_XSD_PATH . "mpoint.xsd" ) === true && count ( $obj_DOM->{'initialize-payment'} ) > 0) {
		...
		for($i = 0; $i < count ( $obj_DOM->{'initialize-payment'} ); $i ++) {
			// Set Global Defaults
			if (empty ( $obj_DOM->{'initialize-payment'} [$i] ["account"] ) === true || intval ( $obj_DOM->{'initialize-payment'} [$i] ["account"] ) < 1) {
				$obj_DOM->{'initialize-payment'} [$i] ["account"] = - 1;
			}
			...
			if ($code == 100) {
				...
				if ($obj_ClientConfig->getUsername () == trim ( $_SERVER ['PHP_AUTH_USER'] ) && $obj_ClientConfig->getPassword () == trim ( $_SERVER ['PHP_AUTH_PW'] ) && $obj_ClientConfig->hasAccess ( $_SERVER ['REMOTE_ADDR'] ) === true) {
					...
					if (($obj_CountryConfig instanceof CountryConfig) === false || $obj_CountryConfig->getID () < 1) {
					...
					if ($obj_ClientConfig->getMaxAmount () > 0 && $iValResult != 10) {
						$aMsgCds [$iValResult + 50] = ( string ) $obj_DOM->{'initialize-payment'} [$i]->transaction->amount;
					}
					...
					if (strlen ( $obj_ClientConfig->getSalt () ) > 0 && (strlen ( $obj_DOM->{'initialize-payment'} [$i]->transaction ['order-no'] ) > 0 || intval ( $obj_DOM->{'initialize-payment'} [$i]->transaction->amount ) > 100 || count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->hmac ) == 1)) {
						...
						if ($obj_Validator->valHMAC ( trim ( $obj_DOM->{'initialize-payment'} [$i]->transaction->hmac ), $obj_ClientConfig, $obj_ClientInfo, trim ( $obj_DOM->{'initialize-payment'} [$i]->transaction ['order-no'] ), intval ( $obj_DOM->{'initialize-payment'} [$i]->transaction->amount ), intval ( $obj_DOM->{'initialize-payment'} [$i]->transaction->amount ["country-id"] ) ) != 10) {
							$aMsgCds [210] = trim ( $obj_DOM->{'initialize-payment'} [$i]->transaction->hmac );
						}
					}
					...
					if (count ( $aMsgCds ) == 0) {
						...
						try {
							...
							if (intval ( $data ['operator'] ) == 0) {
								$data ['operator'] = intval ( $obj_DOM->{'initialize-payment'} [$i]->{'client-info'}->mobile ["country-id"] ) * 100;
							}
							...
							if (count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->{'callback-url'} ) == 1) {
								...
								$data ['callback-url'] = $obj_ClientConfig->getCallbackURL ();
							}
							if (count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->{'auth-url'} ) == 1) {
								...
								$data ['auth-url'] = $obj_ClientConfig->getAuthenticationURL ();
							}
							$data ['icon-url'] = "";
							...
							if (count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->orders ) == 1 && count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->orders->children () ) > 0) {
								...
								for($j = 0; $j < count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->orders->{'shipping-address'} ); $j ++) {
									...
									if ($order_id != "") {
										$data ['shipping_address'] [$j] ['external-id'] = $order_id;
									}
								}
							...
							if (count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->{'custom-variables'} ) == 1 && count ( $obj_DOM->{'initialize-payment'} [$i]->transaction->{'custom-variables'}->children () ) > 0) {
								...
								foreach ( $obj_DOM->{'initialize-payment'} [$i]->transaction->{'custom-variables'}->children () as $obj_Var ) {
									if (substr ( $obj_Var->getName (), 0, 4 ) == "var_") {
										...
										$aVars ["var_" . $obj_Var->getName ()] = ( string ) $obj_Var;
									}
								}
								// Log additional data
							...
							for($j = 0; $j < count ( $obj_XML->item ); $j ++) {
								...
								if ($obj_XML->item [$j] ["type-id"] != 11 || ($obj_TxnInfo->getAccountID () > 0 && (count ( $aObj_XML ) > 0 || $obj_ClientConfig->getStoreCard () == 2))) {
