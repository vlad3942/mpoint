							...
							if ($code >= 10)
							{
									...
									&& (count($obj_DOM->{'delete-card'}[$i]->{'auth-url'}) == 1 || strlen($obj_ClientConfig->getAuthenticationURL() ) > 0) )
								{
									...
									if ($obj_Validator->valURL($url, $obj_ClientConfig->getAuthenticationURL() ) == 10)
									{
                                        ...
                                        if (empty($obj_Customer["customer-ref"]) === true && count($obj_DOM->{'delete-card'}[$i]->{'client-info'}->{'customer-ref'}) > 0) {
                                            $obj_Customer["customer-ref"] = (string) $obj_DOM->{'delete-card'}[$i]->{'client-info'}->{'customer-ref'};
                                        }
								...
								else
								{
									...
									if ($code == 3)
									{
										...
										$_OBJ_TXT = new api\classes\core\TranslateText(array(sLANGUAGE_PATH . $obj_DOM->{'delete-card'}[$i]->{'client-info'}["language"] ."/global.txt", sLANGUAGE_PATH . $obj_DOM->{'delete-card'}[$i]->{'client-info'}["language"] ."/custom.txt"), sSYSTEM_PATH, 0, "UTF-8");
										$obj_mPoint = new EndUserAccount($_OBJ_DB, $_OBJ_TXT, $obj_ClientConfig);
