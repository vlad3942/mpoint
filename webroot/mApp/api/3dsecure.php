...
try
{
	...
	$root = Validate::valRequestFormat($obj_DOM, "mpoint.xsd", "request-3dsecure");

	$code = Validate::valBasic($_OBJ_DB, (integer) $root["client-id"], (integer) $root["account"]);
	if ($code != 100) { throw new mPointSecurityException($code); }

	$obj_ClientConfig = ClientConfig::authenticate($_OBJ_DB, (integer) $root["client-id"], (integer) $root["account"], $_SERVER["PHP_AUTH_USER"], $_SERVER["PHP_AUTH_PW"], $_SERVER["REMOTE_ADDR"]);
