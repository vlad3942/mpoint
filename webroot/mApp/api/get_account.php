			...
			if ($code == 100)
			{
				...
				if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW']) )
				{
					...
					if (count($aMsgCds) == 0)
					{
						...
						$iAccountID = $obj_mPoint->getAccountID($_OBJ_DB, $obj_ClientConfig, $obj_DOM->{'get-account'}[$i]->mobile, $obj_CountryConfig);
						if ($iAccountID < 0) { $iAccountID = $obj_mPoint->getAccountID($_OBJ_DB, $obj_ClientConfig, $obj_DOM->{'get-account'}[$i]->email, $obj_CountryConfig); }
