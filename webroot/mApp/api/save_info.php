		...
		for ($i=0; $i<count($obj_DOM->{'personal-info'}); $i++)
		{
			...
			if ($code == 100)
			{
				...
				if ($obj_ClientConfig->getUsername() == trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() == trim($_SERVER['PHP_AUTH_PW']) )
				{
					...
					if (count($aMsgCds) == 0)
					{
						$iAccountID = EndUserAccount::getAccountID_Static($_OBJ_DB, $obj_ClientConfig, $obj_CountryConfig, $obj_DOM->{'personal-info'}[$i]->{'client-info'}->{'customer-ref'}, $obj_DOM->{'personal-info'}[$i]->{'client-info'}->mobile, $obj_DOM->{'personal-info'}[$i]->{'client-info'}->email);
						...
						if ($code >= 10)
						{
							...
							else
							{
								...
								if ($code == 3)
								{
									...
									$_OBJ_TXT = new api\classes\core\TranslateText(array(sLANGUAGE_PATH . $obj_DOM->{'personal-info'}[$i]->{'client-info'}["language"] ."/global.txt", sLANGUAGE_PATH . $obj_DOM->{'personal-info'}[$i]->{'client-info'}["language"] ."/custom.txt"), sSYSTEM_PATH, 0, "UTF-8");
