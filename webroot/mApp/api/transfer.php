						...
						if ($code >= 10)
						{
							...
							else
							{
								...
								if ($code == 3)
								{
									...
									$_OBJ_TXT = new api\classes\core\TranslateText(array(sLANGUAGE_PATH . $obj_DOM->transfer[$i]->{'client-info'}["language"] ."/global.txt", sLANGUAGE_PATH . $obj_DOM->transfer[$i]->{'client-info'}["language"] ."/custom.txt"), sSYSTEM_PATH, 0, "UTF-8");
									$obj_mPoint = new EndUserAccount($_OBJ_DB, $_OBJ_TXT, $obj_ClientConfig);
									$obj_mPoint->sendAccountDisabledNotification(GoMobileConnInfo::produceConnInfo($aGM_CONN_INFO), $obj_DOM->transfer[$i]->{'client-info'}->mobile);
								}
