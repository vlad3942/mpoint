		...
		for ($i=0, $iMax = count($obj_DOM->pay); $i< $iMax; $i++)
		{
			...
			if (count($aMsgCds) === 0)
			{
				...
				// Client successfully authenticated
 				if ($obj_ClientConfig->hasAccess($_SERVER['HTTP_X_FORWARDED_FOR']) === true && $obj_ClientConfig->getUsername() === trim($_SERVER['PHP_AUTH_USER']) && $obj_ClientConfig->getPassword() === trim($_SERVER['PHP_AUTH_PW']))
                 {
