...
for($i = 0; $i < count ( $obj_DOM->void ); $i ++) {
	...
	if (array_key_exists ( "PHP_AUTH_USER", $_SERVER ) === true && array_key_exists ( "PHP_AUTH_PW", $_SERVER ) === true) {
		if (($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate ( sPROTOCOL_XSD_PATH . "mpoint.xsd" ) === true && count ( $obj_DOM->{'void'} ) > 0) {
			...
			if ($code < 10) {
				$aMsgCodes [$clientID] [] = new BasicConfig ( $code + 10, "Validation of Client : " . $clientID . " failed" );
			} elseif ($code < 20) {
				$aMsgCodes [$clientID] [] = new BasicConfig ( $code + 10, "Validation of Account : " . $account . " failed" );
			}
			...
			if (count ( $aMsgCodes ) == 0) {
				...
				if (Validate::valBasic ( $_OBJ_DB, $clientID, $account ) == 100) {
					...
					// Validate input
					if ($obj_Validator->valUsername ( $usernm ) != 10) {
						$aMsgCds [$obj_Validator->valUsername ( $usernm ) + 20] = $usernm;
					}
					if ($obj_Validator->valPassword ( $passw ) != 10) {
						$aMsgCds [$obj_Validator->valPassword ( $passw ) + 30] = $passw;
					}
					...
					if (count ( $aMsgCds ) == 0) {
						...
						/* ========== Input Validation Start ========== */
						if ($obj_Validator->valPrice ( $obj_TxnInfo->getAmount (), $amount ) != 10) {
							$aMsgCds [$obj_Validator->valPrice ( $obj_TxnInfo->getAmount (), $amount ) + 50] = $amount;
						}
						/* ========== Input Validation End ========== */
								...
								try {
									...
									if ($code == 1000 || $code == 1001) {
										...
										if (strlen ( $obj_TxnInfo->getCallbackURL () ) > 0) {
											$args = array (
													"transact" => $obj_TxnInfo->getExternalID (),
													"amount" => $amount 
											);
