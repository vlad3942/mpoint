...
$HTTP_RAW_POST_DATA .= '<get-client-gomobile-configurations>';
...
{
	if ( ($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate(sPROTOCOL_XSD_PATH ."mconsole.xsd") === true && count($obj_DOM->{'get-client-gomobile-configurations'}) > 0)
	{		
		$obj_val = new Validate();
		$aClientIDs = array();
		for ($i=0; $i<count($obj_DOM->{'get-client-gomobile-configurations'}->clients->{'client-id'}); $i++)
		{
			$aClientIDs[] = (integer) $obj_DOM->{'get-client-gomobile-configurations'}->clients->{'client-id'}[$i];
		}
		...
		case (mConsole::iAUTHORIZATION_SUCCESSFUL):
			...
			foreach ($aClientIDs as $id)
			{
				...
				{
					$i++;
					$xml .= $obj_Config->getClientGoMobileConfigurationToXML($_OBJ_DB);
				}
			}
			...
			else
			{
				...
				$xml = '<client-gomobile-configurations>'. $xml .'</client-gomobile-configurations>';
			}
			break;
		default:
	...
	elseif (count($obj_DOM->{'get-client-gomobile-configurations'}) == 0)
	{
