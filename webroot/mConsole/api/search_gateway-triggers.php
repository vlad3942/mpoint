 ...
 * $HTTP_RAW_POST_DATA .= '<search-gateway-triggers client-id="10007" psp-id="4" />';
 */
...
if (array_key_exists ( "PHP_AUTH_USER", $_SERVER ) === true && array_key_exists ( "PHP_AUTH_PW", $_SERVER ) === true) {
	if (($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate ( sPROTOCOL_XSD_PATH . "mconsole.xsd" ) === true && count ( $obj_DOM->{'search-gateway-triggers'} ) > 0) {
		...
		switch ($code) {
			...
			case (mConsole::iAUTHORIZATION_SUCCESSFUL) :
				...
				try{
					$clientId = $obj_DOM->{'search-gateway-triggers'} {'client-id'};
					$pspId = $obj_DOM->{'search-gateway-triggers'} {'psp-id'};
					$xml = $obj_mPoint->searchGatewayTrigger ( $clientId, $pspId );
				}
				catch (Exception $e)
				{
...
elseif (count ( $obj_DOM->{'search-gateway-triggers'} ) == 0) {
