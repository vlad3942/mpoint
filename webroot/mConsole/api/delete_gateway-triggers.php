 ...
 * $HTTP_RAW_POST_DATA .= '<delete-gateway-triggers client-id="10007" psp-id="4" >';
 * $HTTP_RAW_POST_DATA .= '</delete-gateway-triggers>';
 * $HTTP_RAW_POST_DATA .= '</root>';
 */
...
if (array_key_exists ( "PHP_AUTH_USER", $_SERVER ) === true && array_key_exists ( "PHP_AUTH_PW", $_SERVER ) === true) {
	if (($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate ( sPROTOCOL_XSD_PATH . "mconsole.xsd" ) === true && count ( $obj_DOM->{'delete-gateway-triggers'} ) > 0) {
		...
		$code = $obj_mPoint->singleSignOn ( $obj_ConnInfo, $_SERVER ['HTTP_X_AUTH_TOKEN'], mConsole::sPERMISSION_GET_TRANSACTION_STATISTICS, $aClientIDs , $aClientIDs,$_SERVER['HTTP_VERSION']);
		switch ($code) {
			...
			case (mConsole::iAUTHORIZATION_SUCCESSFUL) :
				...
				try{
				$clientId = $obj_DOM->{'delete-gateway-triggers'}{'client-id'} ;
				$pspId = $obj_DOM->{'delete-gateway-triggers'}{'psp-id'} ;
				$obj_mPoint->deleteGatewayTrigger( $clientId,$pspId);
...
elseif (count ( $obj_DOM->{'delete-gateway-triggers'} ) == 0) {
