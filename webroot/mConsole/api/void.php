...
{	
	if ( ($obj_DOM instanceof SimpleDOMElement) === true && $obj_DOM->validate(sPROTOCOL_XSD_PATH ."mconsole.xsd") === true && count($obj_DOM->{'void'}) > 0)
	{
		...
		$code = $obj_mPoint->singleSignOn($obj_ConnInfo, $_SERVER['HTTP_X_AUTH_TOKEN'], mConsole::sPERMISSION_VOID_PAYMENTS, $aClientIDs);
				...
				for ($i=0; $i<count($obj_DOM->{'void'}->transactions); $i++)
				{
					...
					for ($j=0; $j<count($obj_DOM->{'void'}->transactions[$i]->transaction); $j++)
					{
						$xml .= '<transaction 
									...
									order-no = "'. urlencode($obj_DOM->{'void'}->transactions[$i]->transaction[$j]["ornder-no"]) .'" >';
						...
						if ($code != 200)
						{
							...
							else { trigger_error("Unable to perform refund for Order No.: ". urlencode($obj_DOM->{'void'}->transactions[$i]->transaction[$j]["order-no"]) ." using mPointID: ". intval($obj_DOM->{'void'}->transactions[$i]->transaction[$j]["id"]) .". mPoint returned: ". t ."\n". "Request Body: ". $b ."\n". "Response Body: ". $obj_Client->getReplyBody() ); }
						}
						else 
						{
