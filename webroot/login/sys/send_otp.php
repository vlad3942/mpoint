...
case "form":
	...
	if (empty($xml) === true)
	{
		...
		// Authentication successful - Send One Time Password
		if ($code == 10)
		{
			$obj_XML = simplexml_load_string($obj_mPoint->getAccountInfo($iAccountID) );
			if (floatval($obj_XML->mobile) > $obj_CountryConfig->getMinMobile() )
			{
				$code = $obj_mPoint->sendOneTimePassword(GoMobileConnInfo::produceConnInfo($aGM_CONN_INFO), $iAccountID, (string) $obj_XML->mobile);
				...
				{
					$xml = '<form id="92" name="'. (string) $obj_XML->form['name'] .'">'. htmlspecialchars($_OBJ_TXT->_("send - code: 92"), ENT_NOQUOTES) .'</form>';
				}
			}
			else
			{
				...
				$xml = '<document type="command">
						...
						<document type="status">
							<form id="102" name="'. (string) $obj_XML->form['name'] .'">'. htmlspecialchars($_OBJ_TXT->_("auth - code: 102"), ENT_NOQUOTES) .'</form>
						</document>
		...
		{
			$xml = '<form id="'. ($code + 90) .'" name="'. (string) $obj_XML->form['name'] .'">'. htmlspecialchars($_OBJ_TXT->_("auth - code: ". ($code + 90) ), ENT_NOQUOTES) .'</form>';
		}
	}
	break;
default:
