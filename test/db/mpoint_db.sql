...
-- card-currency mapping (Presentment-currency) table
CREATE TABLE client.card_currency_mapping_tbl (
	id int4 NOT NULL GENERATED BY DEFAULT AS IDENTITY,
	client_id int4 NOT NULL,
	card_id int4 NOT NULL,
	sale_currency_id int4 NOT NULL,
	is_presentment bool NOT NULL DEFAULT false,
	settlement_currency_id int4 NOT NULL,
	created timestamp NULL DEFAULT now(),
	modified timestamp NULL DEFAULT now(),
	CONSTRAINT pk_client_card_currency_mapping_id PRIMARY KEY (id),
	CONSTRAINT card_currency_mapping_card_fk FOREIGN KEY (card_id) REFERENCES system.card_tbl(id) ON DELETE CASCADE,
	CONSTRAINT card_currency_mapping_sale_currency_fk FOREIGN KEY (sale_currency_id) REFERENCES system.currency_tbl(id) ON DELETE CASCADE,
	CONSTRAINT card_currency_mapping_settlement_currency_fk FOREIGN KEY (settlement_currency_id) REFERENCES system.currency_tbl(id) ON DELETE CASCADE,
	CONSTRAINT client_id_fk FOREIGN KEY (client_id) REFERENCES client.client_tbl(id) ON DELETE CASCADE
)
WITH (
  OIDS=FALSE
);
...
CREATE INDEX cardcurrencymapping_idx ON client.card_currency_mapping_tbl USING btree (client_id, card_id, sale_currency_id);
