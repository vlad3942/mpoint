...
abstract class UpdateSettlementStatusAPITest extends baseAPITest
{
	...
	protected function testSettlementIsAlreadyProcessed($pspID)
    {
		...
		$this->queryDB("INSERT INTO Log.Transaction_Tbl (id, typeid, clientid, accountid, countryid, pspid, extid, orderid, callbackurl, amount, ip, enabled, keywordid, sessionid) VALUES (1001001, 100, 10099, 1100, 100, $pspID, '1515', '1513-005', '', 5000, '127.0.0.1', TRUE, 1, 1)");
		$this->queryDB("INSERT INTO Log.Message_Tbl (txnid, stateid) VALUES (1001001, ". Constants::iPAYMENT_ACCEPTED_STATE. ")");
		$this->queryDB("INSERT INTO Client.AdditionalProperty_Tbl (key, value, externalid, type,scope) VALUES ('UATP_SETTLEMENT_FILE_NAME', 'tsto1654', 10099, 'client',1)");
		$this->queryDB("INSERT INTO client.additionalproperty_tbl (key, value, externalid, type,scope) VALUES ('FILE_EXPIRY', '1', (SELECT id FROM Client.MerchantAccount_Tbl WHERE clientid = 10099 and pspid = $pspID), 'merchant',1)");

		//Add entry in settlement_tbl with "accpeted" status which indicates settlement is already processed 
		$this->queryDB("INSERT INTO Log.settlement_tbl (id,record_number,file_reference_number,file_sequence_number,client_id,psp_id,record_tracking_number,record_type,description,status) VALUES ($alreadyProcessedSettlementId,0,'test1654',$sAlreadyProcessedSettlementDate,10099,$pspID,$sAlreadyProcessedSettlementDate,'CAPTURE',$sAlreadyProcessedSettlementDate,'accepted');");

		//Add entry in settlement_tbl with "waiting" status which indicates settlement is going to process
		$this->queryDB("INSERT INTO Log.settlement_tbl (id,record_number,file_reference_number,file_sequence_number,client_id,psp_id,record_type,status) VALUES ($currentProcessingSettlementId,0,'test1654',$sCurrentProcessingSettlementDate,10099,$pspID,'CAPTURE','waiting');");
