...
//		$xml .= '<cryptogram type="3ds">AKh96OOsGf2HAIDEhKulAoABFA==</cryptogram>';
	...
	protected function testSuccessfulAuthorize($pspID)
	{
		...
		if ($pspID == Constants::iNETAXEPT_PSP) { $this->assertTrue(in_array(Constants::iCARD_PURCHASE_TYPE, $aStates)); }
	...
	protected function testSuccessfulAuthorizeIncludingAutoCapture($pspID)
	{
        ...
        { //On local Google Pub is not setup so callback will fail
            $this->assertTrue(in_array(Constants::iCB_REJECTED_STATE, $aStates));
        }
		else
        {
            $this->assertTrue(in_array(Constants::iCB_ACCEPTED_STATE, $aStates));
		...
		$this->assertTrue(in_array(Constants::iPAYMENT_CAPTURED_STATE, $aStates));
