...
class RoutingServiceTest extends baseAPITest
{
    ...
    protected function getInitDoc($client, $account, $country, $currency = null, $sessionId=null,$paymentGroupCode=null)
    {
        ...
        $xml .= '</transaction>';
        if(empty($paymentGroupCode)===false)
        {
            $xml .= '<payment_group_code>'.$paymentGroupCode.'</payment_group_code>';
        }
    ...
    public function testGetWalletPaymentMethods()
    {
        ...
        $this->queryDB("INSERT INTO log.session_tbl (id, clientid, accountid, currencyid, countryid, stateid, orderid, amount, mobile, deviceid, ipaddress, externalid, sessiontypeid) VALUES (10, 10099, 1101, 208, 100, 4001, 'Wallet103-1418291', 5000, 9876543210, '', '127.0.0.1', -1, 1);");
        $this->queryDB("INSERT INTO Log.Transaction_Tbl (id, typeid, clientid, accountid, keywordid, pspid, cardid, euaid, countryid, orderid, callbackurl, amount, ip, enabled,sessionid,convertedamount) VALUES (1001001, 100, 10099, 1101, 1,  18, 8, 5001, 100, 'Wallet103-1418291', 'test.com', 5000, '127.0.0.1', TRUE,10,5000)");
        $this->queryDB("INSERT INTO Log.Transaction_Tbl (id, typeid, clientid, accountid, keywordid, pspid, cardid, euaid, countryid, orderid, callbackurl, amount, ip, enabled,sessionid,convertedamount) VALUES (1001002, 100, 10099, 1101, 1,  18, 8, 5001, 100, 'Wallet103-1418291', 'test.com', 5000, '127.0.0.1', TRUE,10,5000)");
...
VALUES(1101, 14, 'paymaya acq',  true)");
        ...
        $xml = $this->getInitDoc(10099, 1101, 100, 208, 10);
    ...
    public function testGetPaymentMethodsWithPaymentGroupCode()
    {
        $this->queryDB("INSERT INTO Client.Client_Tbl (id, flowid, countryid, name, username, passwd) VALUES (10099, 1, 100, 'Test Client', 'Tuser', 'Tpass')");
        $this->queryDB("INSERT INTO Client.URL_Tbl (clientid, urltypeid, url) VALUES (10099, 4, 'http://mpoint.local.cellpointmobile.com/')");
        $this->queryDB("INSERT INTO Client.Account_Tbl (id, clientid) VALUES (1100, 10099)");
        ...
        $xml = $this->getInitDoc(10099, 1100, 100,null,null,'AV001');
        $obj_DOM = simpledom_load_string($xml);

        $iTxnID = 1001001;
        $obj_TxnInfo = TxnInfo::produceInfo($iTxnID, $this->_OBJ_DB);
        $obj_ClientInfo = ClientInfo::produceInfo($obj_DOM->{'initialize-payment'}->{'client-info'}, CountryConfig::produceConfig($this->_OBJ_DB, (integer) $obj_DOM->{'initialize-payment'}->{'client-info'}->mobile["country-id"]), $_SERVER['HTTP_X_FORWARDED_FOR']);
        $obj_RS = new RoutingService($obj_TxnInfo, $obj_ClientInfo, $this->_aHTTP_CONN_INFO['routing-service'], $obj_DOM->{'initialize-payment'}["client-id"], $obj_DOM->{'initialize-payment'}->transaction->amount["country-id"], null, $obj_DOM->{'initialize-payment'}->transaction->amount,null,null,null,null,null,(string)$obj_DOM->{'initialize-payment'}->{'payment-group-code'});

        $this->assertInstanceOf(RoutingService::class, $obj_RS);
        if($obj_RS instanceof RoutingService)
        {
            $getMockMethod = self::getReflectionMethod('RoutingService', 'getPaymentMethodSearchCriteriaXml');
            self::assertStringNotContainsString(" <payment-group-code>AV001</payment-group-code>", $getMockMethod->invoke($obj_RS));
        } else {
            self::fail();
        }
    }
