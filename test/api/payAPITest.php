...
class PayAPITest extends mPointBaseAPITest
{
	...
	protected function testSuccessfulPay($pspID)
	{
		...
		$this->queryDB("INSERT INTO Client.CardAccess_Tbl (clientid, cardid, pspid) VALUES (113, 2, $pspID)"); //Card 2
		$this->queryDB("INSERT INTO System.CardPricing_Tbl (pricepointid, cardid) SELECT C.id * -1 AS pricepointid, 2 FROM System.Country_Tbl C, System.Card_Tbl Card WHERE C.id = 100 GROUP BY pricepointid;");
		...
		$xml = $this->getPayDoc(113, 1100, 1001001, 2);
		...
		$this->assertContains('<?xml version="1.0" encoding="UTF-8"?><root><psp-info id="2" merchant-account="4216310">', $sReplyBody);
	}
