...
class RefundCallbackAPITest extends baseAPITest
{
    ...
    protected function getCallbackDoc($transactionId, $orderId)
    {
        ...
        $xml .= '<card type-id="8" />';
        $xml .= '</transaction>';
        $xml .= '<status code="2003">The resource was successfully created.</status>';
        $xml .= '<approval-code>928201</approval-code>';
        $xml .= '</callback>';
        $xml .= '</root>';
    ...
    public function testSuccessfulCallbackAccepted()
    {
        ...
        $this->queryDB("INSERT INTO log.message_tbl (txnid, stateid) VALUES (1001001, 2003)");
